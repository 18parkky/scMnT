...
try:
    ...
    import scMnT_utility as scMnT_utility
...
def prepare_STR_chunks( PATH_str_tsv, PATH_bam, mapq_threshold, filename, threads, start_time, PATH_multiprocess_out ):
    ...
    logging.info(f"Processing {len(dict_locus_to_nonzero_readcount)} loci instead of {len(STR_table)} loci (elapsed time: {scMnT_utility.getElapsedTime(start_time)} seconds)")
    STR_table = STR_table[(STR_table['locus'].isin( dict_locus_to_nonzero_readcount.keys() ))].copy()
    
    logging.info(f"[scMnT-getAlleleTable-{filename}] Processing {len(dict_locus_to_nonzero_readcount)} loci instead of {loci_count} loci (elapsed time: {scMnT_utility.getElapsedTime(start_time)} seconds)")
...
def runGetAlleleTable( PATH_bam, PATH_str_tsv, PATH_reference_genome, mapq_threshold, threads, flanking_length, realignment_flanking_length, start_time, filename, DIR_out ):
    ...
    try:
        ...
        logging.error(f"[scMnT-getAlleleTable-{filename}] Number of reads ({len(AlleleTable)}) is less than number of threads to use! Multiprocessing won't be used for error-correction.")
