...
MS loci files for mononucleotide-repeat regions (≥10 bp) in the human genome (GRCh38 and T2T-CHM13) are available in scMnT GitHub repository under:`~/scMnT/ref/`.
scMnT has been tested in Python version 3.12.5. and is expected to work in later versions. 
...
**For users who have to work with multiple samples (each from different sequencing runs, it is important to run scMnT for each sample separately, and then merge the ouptut files.**

### 1) `scMnT`
The `scMnT` command runs the entire scMnT workflow, from (1) collecting reads that map to MS loci and (2) labeling MS information for each cell onto the user-provided Scanpy object (h5ad). `scMnT` requires the following files as inputs. 
- Scanpy object where cell type information is available at .obs['CellType'] and cell barcodes (e.g., TAGTTGGCAGGTCCAC-1) must be available at .obs.index
