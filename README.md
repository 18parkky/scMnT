...
Each step can also be called separately: (1) `getAlleleTable`, (2) `scMnT-score`, (3) `scMnT-find` (experimental).

### 1) `scMnT`
The `scMnT` command runs the entire scMnT workflow, from (1) collecting reads that map to MS loci (getAlleleTable), (2) labeling MS information for each cell onto the user-provided Scanpy object (scMnT-score), then (3) identifying MSI cell types by comparing MSI score distributions (scMnT-find). `scMnT` requires the following files as inputs. **In most cases, running this command is all that’s needed.** 
- Scanpy object where cell type (or any other categorical cell annotation, e.g., Leiden clusters) information is available at .obs['CellType'] and cell barcodes (e.g., TAGTTGGCAGGTCCAC-1) must be available at .obs.index. Please ensure that each cell type has enough number of cells (>100). Tumor cells must be labeled 'Tumor' (e.g., Tumor 1, Tumor_2, Tumor III), but if you don't know which cells are tumor cells, you can omit this information (see FAQ).
- Single sample as input (Recommended): a BAM where each read is tagged by its CB (CB tag) and UMI (UB tag)
- Multiple samples as input (i.e., when you're analyzing multiple samples as a single Scanpy Obj): a sample sheet tab-separated file (TSV) file that provides two columns: one for name of each sample and one for specifying the path to its bamfile. The reads in the bam file must have CB and UMI tagged. Importantly, the Scanpy object must have sample name information available at .obs['SampleID'], and this must match the sample sheet TSV file! Only use this option if your Scanpy object is composed of multiple samples.
...
2. What if I don't know which cells are tumor cells in my single-cell data?
  - scMnT-find requires users to specify which cell clusters correspond to tumor populations by labeling them as Tumor (e.g., Tumor_1, Tumor_2, Tumor_3). However, tumor cell identities may not always be known a priori. In such cases, users can inspect the MSI score–annotated Scanpy object generated by the scMnT command. By integrating MSI scores with transcriptomic or genomic features (e.g., marker gene expression, inferCNV scores), tumor populations can be manually annotated. The scMnT-find command can then be re-run using the updated Scanpy object to identify cell populations that are statistically significant enough to be classified as MSI.
